 ...
 * scripts/scrape.js
 *
 * Scrapes TOP 14 results, standings, and calendar from the LNR website.
 * Outputs data/scraped.json with structured intermediate data for the
 * pipeline (validate.js -> elo.js -> generate.js).
 *
 ...
 *
 * Error handling:
 *   - console.error() + process.exit(1) on complete failure
 *   - console.warn() on partial data (incomplete matchday)
 */
...
async function main() {
  ...
  // 1. Fetch results & calendar page
  let resultsHtml;
  try {
    resultsHtml = await fetchPage(RESULTS_URL);
  } catch (err) {
    console.error(`Failed to fetch results page: ${err.message}`);
    process.exit(1);
  }

  // 2. Parse results & calendar
  let matchData;
  try {
    matchData = parseResultsAndCalendar(resultsHtml);
  } catch (err) {
    console.error(`Failed to parse results: ${err.message}`);
    process.exit(1);
  }

  // 3. Fetch & parse standings (best-effort — may be empty if JS-rendered)
  let standings = [];
  try {
    ...
    console.warn('Standings will be empty — pipeline can compute from results');
  }

  if (standings.length === 0) {
    console.warn(
      'Standings table not found (likely JS-rendered). ' +
        'Standings will be empty in scraped output.',
    );
  ...
  if (!matchData.complete) {
    console.warn(
      `Matchday ${matchData.matchday} is incomplete: ` +
        `${matchData.results.length} results, ` +
        `${matchData.calendar.length} upcoming matches ` +
        `(expected ${MATCHES_PER_MATCHDAY} total results)`,
    );
  }

  // 5. Validate team IDs in results
  ...
  console.log(`  Source: LNR (top14.lnr.fr)`);
  console.log(`  Matchday: ${output.matchday}`);
  console.log(`  Complete: ${output.complete}`);
  console.log(`  Standings: ${standings.length} teams`);
  console.log(`  Results: ${matchData.results.length} matches`);
  console.log(`  Calendar: ${matchData.calendar.length} upcoming matches`);
